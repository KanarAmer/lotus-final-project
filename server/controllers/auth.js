...
const signup = async (req, res) => {
    const {email, password, name, lastMenstruation} = req.body.user
    const hashedPassword = await bcrypt.hash(password, 10)
    if(!email || !password || !name) return resWithStatus(res, false, 400, "one of the fields is empty.")
    const result = await DBactions.createUser({email, password: hashedPassword, name, lastMenstruation})
...
const signin = async (req, res) => {
    ...
    return resWithStatus(res, true, 200, {token: accessToken, userId: user.userId})
}

//handle update details request - input validation, hashing the new password if given, and dispatch the db action.
const updateDetails = async (req, res) => {
    const {email, name, password, _id, lastMenstruation} = req.body
    if(!email || !password || !name || !lastMenstruation) return resWithStatus(res, false, 400, "one of the fields is empty.")
    let update = {email, name, lastMenstruation}
    if(password.update) {
